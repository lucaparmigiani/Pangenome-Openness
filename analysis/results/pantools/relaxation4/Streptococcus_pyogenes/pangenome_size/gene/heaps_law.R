#! /usr/bin/env RScript
df1 <- read.csv("/vol/data/Pantools/db/Streptococcus_pyogenes/pangenome_size/gene/gene_nrs_for_heaps_law.csv", header=FALSE)
df2 <- read.csv(file = "/vol/data/Pantools/db/Streptococcus_pyogenes/pangenome_size/gene/genes_for_heaps_law.csv", header = FALSE)
...
sink("/vol/data/Pantools/db/Streptococcus_pyogenes/pangenome_size/gene/heaps_law_alpha.txt")
cat(fit$par[2])
sink()
cat("alpha =", fit$par[2], "\n\n")