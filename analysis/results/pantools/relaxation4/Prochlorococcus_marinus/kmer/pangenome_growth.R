...
df = read.csv("/vol/data/Pantools/db/Prochlorococcus_marinus/pangenome_size/kmer/core_accessory_size.csv", sep=",", header = TRUE)
...
pdf(NULL)
ggsave("/vol/data/Pantools/db/Prochlorococcus_marinus/pangenome_size/kmer/core_accessory_growth.png", plot= plot1, height = 10, width = 10)
cat("\nPangenome growth written to: /vol/data/Pantools/db/Prochlorococcus_marinus/pangenome_size/kmer/core_accessory_growth.png\n\n")
...
df = read.csv("/vol/data/Pantools/db/Prochlorococcus_marinus/pangenome_size/kmer/core_accessory_unique_size.csv", sep=",", header = TRUE)
#df <- mutate(df,y = y / 10^6) # convert scientific number to millions (requires dplyr library)
plot1 = ggplot(df, aes(x=x, y=y, color=Category, size=Category)) +
    scale_color_manual(values=c("Median" = "#000000", "Accessory" = "#0066CC", "Core" = "#009900", "Unique" = "#990000")) +
    scale_size_manual(values=c("Median" = 0.2, "Accessory" = 0.8, "Core" = 0.8, "Unique" = 0.8)) +
...
pdf(NULL)
ggsave("/vol/data/Pantools/db/Prochlorococcus_marinus/pangenome_size/kmer/core_accessory_unique_growth.png", plot= plot1, height = 10, width = 10)
cat("\nPangenome growth written to: /vol/data/Pantools/db/Prochlorococcus_marinus/pangenome_size/kmer/core_accessory_unique_growth.png\n\n")
